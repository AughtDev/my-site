import {visit} from 'unist-util-visit'
import {ElementContent,Element as HastElement, Root} from "hast";


const CLIPBOARD_SVG: ElementContent = {
    type: 'element',
    tagName: 'svg',
    properties: {
        xmlns: 'http://www.w3.org/2000/svg',
        viewBox: '0 0 24 24',
        fill: 'none',
        stroke: 'currentColor',
        'stroke-width': '1.152',
        'stroke-linecap': 'round',
        'stroke-linejoin': 'round',
        className: ["copy-svg not-copied"]
    },
    children: [
        {
            type: 'element',
            tagName: 'path',
            properties: {
                d: "M16 4C16.93 4 17.395 4 17.7765 4.10222C18.8117 4.37962 19.6204 5.18827 19.8978 6.22354C20 6.60504 20 7.07003 20 8V17.2C20 18.8802 20 19.7202 19.673 20.362C19.3854 20.9265 18.9265 21.3854 18.362 21.673C17.7202 22 16.8802 22 15.2 22H8.8C7.11984 22 6.27976 22 5.63803 21.673C5.07354 21.3854 4.6146 20.9265 4.32698 20.362C4 19.7202 4 18.8802 4 17.2V8C4 7.07003 4 6.60504 4.10222 6.22354C4.37962 5.18827 5.18827 4.37962 6.22354 4.10222C6.60504 4 7.07003 4 8 4M9.6 6H14.4C14.9601 6 15.2401 6 15.454 5.89101C15.6422 5.79513 15.7951 5.64215 15.891 5.45399C16 5.24008 16 4.96005 16 4.4V3.6C16 3.03995 16 2.75992 15.891 2.54601C15.7951 2.35785 15.6422 2.20487 15.454 2.10899C15.2401 2 14.9601 2 14.4 2H9.6C9.03995 2 8.75992 2 8.54601 2.10899C8.35785 2.20487 8.20487 2.35785 8.10899 2.54601C8 2.75992 8 3.03995 8 3.6V4.4C8 4.96005 8 5.24008 8.10899 5.45399C8.20487 5.64215 8.35785 5.79513 8.54601 5.89101C8.75992 6 9.03995 6 9.6 6Z",
                'stroke-width': "1.152",
                'stroke-linecap': "round",
                'stroke-linejoin': "round"
            },
            children: []
        }
    ]
}

const CHECKMARK_SVG: ElementContent = {
    type: 'element',
    tagName: 'svg',
    properties: {
        xmlns: 'http://www.w3.org/2000/svg',
        viewBox: '0 0 24 24',
        fill: 'none',
        stroke: 'currentColor',
        'stroke-width': '1.152',
        'stroke-linecap': 'round',
        'stroke-linejoin': 'round',
        className: ["copy-svg copied"]
    },
    children: [
        {
            type: 'element',
            tagName: 'path',
            properties: {
                d: "M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"
            },
            children: []
        },
        {
            type: 'element',
            tagName: 'path',
            properties: {
                d: "M15 2H9a1 1 0 00-1 1v2a1 1 0 001 1h6a1 1 0 001-1V3a1 1 0 00-1-1z"
            },
            children: []
        },
        {
            type: 'element',
            tagName: 'path',
            properties: {
                d: "M9 13l2 2 4-4"
            },
            children: []
        }
    ]
};

export default function rehypeCodeMeta() {
    return (tree: Root) => {
        visit(tree, 'element', (node) => {
            if (node.tagName !== 'pre') return
            const lang = node.properties['data-language'];
            if (!lang) return;

            const langDiv: ElementContent = {
                type: 'element',
                tagName: 'div',
                properties: {className: ['code-lang']},
                children: [{type: 'text', value: lang.toString()}]
            }

            // const copyBtn = {
            //     type: 'element',
            //     tagName: 'button',
            //     properties: {className: ['copy-btn'], title: 'Copy code'},
            //     children: [
            //         {
            //             type: 'element',
            //             tagName: 'svg',
            //             properties: {
            //                 xmlns: 'http://www.w3.org/2000/svg',
            //                 viewBox: '0 0 24 24',
            //                 fill: 'none',
            //                 stroke: 'currentColor',
            //                 'stroke-width': '2',
            //                 'stroke-linecap': 'round',
            //                 'stroke-linejoin': 'round',
            //                 className: ['feather', 'feather-copy']
            //             },
            //             children: [
            //                 {
            //                     type: 'element',
            //                     tagName: 'rect',
            //                     properties: {
            //                         x: '9',
            //                         y: '9',
            //                         width: '13',
            //                         height: '13',
            //                         rx: '2',
            //                         ry: '2'
            //                     },
            //                     children: []
            //                 },
            //                 {
            //                     type: 'element',
            //                     tagName: 'path',
            //                     properties: {d: 'M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1'},
            //                     children: []
            //                 }
            //             ]
            //         }
            //     ]
            // };

            // find button
            let copyBtn: ElementContent | null = null;
            const code_block = (node.children.find((child) => child.type === 'element' && child.tagName === 'code') as HastElement | undefined)
            if (code_block) {
                const oldCopyBtn = Array.from(code_block.children).find((child) => (child as HastElement).tagName === 'button')
                if (oldCopyBtn) {
                    copyBtn = {
                        type: 'element',
                        tagName: 'button',
                        properties: {
                            ...(oldCopyBtn as HastElement).properties,
                            className: ['copy-btn']
                        },
                        // 2 svgs, a clip board and a check mark
                        children: [CLIPBOARD_SVG, CHECKMARK_SVG]
                    };
                    // remove old button
                    (oldCopyBtn as HastElement).properties["style"] = "display: none;"

                }
            }

            const horizontalDivider: ElementContent = {
                type: 'element',
                tagName: 'hr',
                properties: {className: ['code-divider']},
                children: []
            };

            const wrapperDiv: ElementContent = {
                type: 'element',
                tagName: 'div',
                properties: {className: ['code-header']},
                children: copyBtn ? [langDiv,copyBtn] : [langDiv]
            };

            node.children.unshift(wrapperDiv);
            node.children.splice(1, 0, horizontalDivider);
        })

    }
}

